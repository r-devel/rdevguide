...
8. Include (mostly as the first sentence), a to-the-point explanation of what the purpose of the patch is. This sentence should not be in the descriptive form, rather an imperative form will be more suitable here. If this is not enough detail for a patch, a new paragraphs(s) can be added to explain in proper depth what has happened. The details should be good enough that a core developer reading it understands the justificationfor the change.

9. If your patch is in response to a pre-existing issue or bug report on Bugzilla, then please make sure to reference the bug number using `Bug-NNNNN` in the patch title or message or comment.

10. If the patch is for an unreported issue or bug (assuming you already performed a search on Bugzilla for a pre-existing issue or bug), create a new bug report and include your patch with it. Please fill in as much relevant detail as possible to prevent reviewers from having to delay reviewing your patch because of lack of information.
...
5. is reproducible on following the stated steps. If not then, you may comment which step is not reproducible on the version of R that you are using.
...
Once your patch has reached an acceptable state (and thus considered 'accepted'), it will either be applied or rejected. If it is rejected, please do not take it personally! Your work is still appreciated regardless of whether your patch is applied. Balancing what does and does not go into R is tricky and everyone's contributions cannot always be accepted.
